...
truth model; it is not evidence that the filter is conservative for real sensors. The
NEES epochs are autocorrelated, so the effective sample count is far below the nominal
57,600.
...
ECEF position retains **3,189,068.5 m radial error** (an analytic annotation, not a
measurement). Horizontal replay scoring at the
...
Second, after replacing that confounded path with an effectively exact prior, prior-only
velocity RMS is **0.2506 m/s**; default Doppler raises
...
- The reviewer's independent extended Q sweep gives
  0.2659/0.2616/0.2398/0.2365/0.3084/0.3753/0.6643 m/s across 4e-7..0.4,
  independently confirming that Q=4e-4 is a genuine interior optimum.
...
Q=4e-4 empirically minimizes velocity RMS at **0.2365 m/s**, better than prior-only and
a genuine interior optimum. This Doppler-degrades-velocity result is contingent on the
near-truth IMU (propagation error approximately 5e-4 m/s²). At sea with realistic IMU
error the sign could flip and the low-Q fix could be wrong. This is therefore a
synthetic-stack tuning, not a value to freeze or a general D39 result. The replay
initialization defect must be fixed before production tuning.
...
than converges. The 20-minute crossover is fragile: it rests on means from only six seeds.
The maneuver-reset question is unfalsifiable by construction here because the inserted
turn enters through the near-truth IMU; this is a harness limitation, not evidence about
the real filter. The observed windows are 28.52 m before the turn, 43.87 m just after, and
125.24 m at the end. The stub also has no heading-to-velocity coupling or manoeuvre
covariance reset.
...
0/99.83/100/100%. Epoch shifting aliases orbital phase, hence the non-monotonic innovation
RMS; the nonzero innovations are roughly 3000–5000 times the gate. These data support only
that every tested nonzero staleness (>=1 h) is rejected, not the 6 h choice. Because the
test omits the HPH' term, rejection is an upper bound. It does **not** validate actual
SupGP error growth; the first tested nonzero offset already rejects.
