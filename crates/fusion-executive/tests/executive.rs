...
use pnt_types::{
    Frame, GnssFix, ImuSample, MeasurementEnvelope, MeasurementPayload, Provenance,
    QualityFlags, SourceId, TimeTag,
};
...
fn recorded_only_sends_gnss_to_truth_but_not_fusion() {
    let mut executive = Executive::new(
        Config { gnss_authority: GnssAuthority::RecordedOnly },
        ManualClock::default(), FilterStub::default(), IntegrityStub, MemoryJournals::default(),
    );
...
fn authority_modes_change_routing_table_not_processing_graph() {
    let production = Executive::<ManualClock, FilterStub, IntegrityStub, MemoryJournals>::routing_table(GnssAuthority::Production);
    let recorded = Executive::<ManualClock, FilterStub, IntegrityStub, MemoryJournals>::routing_table(GnssAuthority::RecordedOnly);
    let off = Executive::<ManualClock, FilterStub, IntegrityStub, MemoryJournals>::routing_table(GnssAuthority::Off);
...
fn covariance_grows_on_every_imu_tick_without_measurements() {
    ...
    for sequence in 1..=3 {
        executive.process(envelope(sequence, MeasurementPayload::Imu(ImuSample { acceleration_mps2: [1.0, 0.0, 0.0], angular_rate_rps: [0.0; 3] })));
    }
...
fn synthetic_imu_and_measurement_emit_a_solution_epoch() {
    ...
    executive.process(envelope(1, MeasurementPayload::Imu(ImuSample { acceleration_mps2: [0.5, 0.0, 0.0], angular_rate_rps: [0.0; 3] })));
