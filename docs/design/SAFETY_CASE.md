...
actually does on `GPS_INPUT` silence is [UNVERIFIED — to be characterised in ArduPilot SITL at
the pinned firmware, per D17 / U-M1].** `GPS_TIMEOUT_MS` (4000 ms) is a GPS-backend *data*
...
| **H5** | **MAVLink link loss mid-authority** | Cable/USB/serial/connector fault. | This is **Case B**: internal revocation cannot reach ArduPilot. ArduPilot's response to `GPS_INPUT` silence is **[UNVERIFIED — to be characterised in SITL at the pinned firmware, per D17 / U-M1]**; it is *not* demonstrated to be a bounded non-manoeuvre action (`GPS_TIMEOUT_MS` is a GPS-backend data timeout, not a proven control-authority action). Present backstop is helm override/kill-cord (link-independent). Companion detects link loss via ArduPilot→companion heartbeat deadline → alarm/log only. | **`uncontrolled-pending-evidence`** (as H6/H7): whether ArduPilot manoeuvres on `GPS_INPUT` silence, and any coast window, are unproven until U-M1 SITL characterisation; only the physical layer is currently relied on. The prior "bounded ~4 s non-manoeuvre timeout" and "`t_lease` guarantees the companion revokes first" claims are withdrawn. **[UNVERIFIED]/uncontrolled — registered §5.** |
...
- **ArduPilot response to `GPS_INPUT` silence (Case B) — [UNVERIFIED], `uncontrolled-pending-evidence`:**
  ...
  behaviour depends on mode, EKF source/validity and configuration). **U-M1** shall characterise
  ...
  Until characterised, Case B is **uncontrolled by present design** (as H6/H7), backed only by
